...
func StartCheck(mapping *upnp.Upnp) {
    if !CheckNet(mapping) {
        fmt.Println("check upnp faild: the gateway device don't support upnp")
        return
    }
    fmt.Println("local ip address: ", mapping.LocalHost)
    ExternalIPAddr(mapping)
}

/*
	检查网络是否支持upnp协议
*/
func CheckNet(mapping *upnp.Upnp) bool {
    err := mapping.SearchGateway()
    if err != nil {
        fmt.Println("an error occoured during CheckNet:", err)
        return false
    } else {
        fmt.Println("check succeed. the gateway support upnp")
        fmt.Println("gateway ip address: ", mapping.Gateway.Host)
        return true
    }
}

//获得公网ip地址
func ExternalIPAddr(mapping *upnp.Upnp) {
    err := mapping.ExternalIPAddr()
    if err != nil {
        fmt.Println(err.Error())
    } else {
        fmt.Println("external ip address: ", mapping.GatewayOutsideIP)
    }
}
